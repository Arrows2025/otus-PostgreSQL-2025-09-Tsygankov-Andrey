...
В результате секционирования удалось уменьшить время выполнения запросов по условию выборки дат или диапазона дат. В перспективе для новых данных потребуется создание и присоединение к секционированной таблице новых секций `ATTACH PARTITION`, а для удаления старых данных или переноса их в архив не потребуется нагружать базу данных операциями вставки и удаления, достаточно отсоединить старую секцию от секционированной таблицы `DETACH PARTITION`, что на порядок быстрее обычных операций манипулирования данными

...
Далее в первой сессии синего окна выполняю UPDATE на строку таблицы Table_B, во второй сессии зелёного окна выполняю UPDATE на строку таблицы Table_C, а в третьей сессии красного окна выполняю UPDATE на строку таблицы Table_A и в третьей сессии получаю взаимоблокировку
```
...
4️⃣ Задание со ⭐: Две транзакции, выполняющие единственную команду UPDATE без оператора WHERE одной и той же таблицы, могут вызвать взаимоблокировку.

Для проверки в первой сессии PID = 102270 синего окна запускаю UPDATE с оператором WHERE таблицы `test_locks` с прямым ORDER BY, а во второй сессии PID = 102363 зелёного окна запускаю UPDATE с оператором WHERE этой же таблицы с обратным ORDER BY. Таблица содержит один миллион строк, и в какой-то момент оба запущенных UPDATE вышли на блокировку одних и тех же строк в конфликтующем режиме, что вызвало взаимоблокировку
```
UPDATE test_locks set Data = Data || '+test3' WHERE id IN (SELECT id FROM test_locks ORDER BY id ASC);
```
<img width="1881" height="431" alt="image" src="https://github.com/user-attachments/assets/5d788f99-39f4-42d3-b069-a3851cec2043" /><br>
```
UPDATE test_locks set Data = Data || '+test4' WHERE id IN (SELECT id FROM test_locks ORDER BY id DESC);
```
<img width="1881" height="551" alt="image" src="https://github.com/user-attachments/assets/13edef98-a32f-4396-98c1-b80882330327" /><br>

<b>Дополнение</b>: Так как в PostgreSQL нет возможности развернуть порядок строк в UPDATE без использования оператора WHERE, пойдём прямым путём. В первой сессии PID = 140002 синего окна запускаю UPDATE без оператора WHERE таблицы `test_locks` без какой-либо сортировки, и во второй сессии PID = 140008 зелёного окна запускаю UPDATE без оператора WHERE этой же таблицы без какой-либо сортировки. Таблица содержит один миллион строк, и в какой-то момент оба запущенных UPDATE вышли на блокировку одних и тех же строк в конфликтующем режиме, что вызвало взаимоблокировку, как и при встречном UPDATE с оператором WHERE
```
UPDATE test_locks set Data = Data || '+test5';
```
<img width="2089" height="461" alt="image" src="https://github.com/user-attachments/assets/9674d142-dfa1-4276-8b0f-ef15fdc9ff36" /><br>
```
UPDATE test_locks set Data = Data || '+test6';
```
<img width="2089" height="431" alt="image" src="https://github.com/user-attachments/assets/285d01ec-eef2-4636-88fc-038253fc729a" /><br>

В журнале сообщений находим информацию о взаимоблокировке
```
ОШИБКА:  обнаружена взаимоблокировка
ПОДРОБНОСТИ:  Процесс 140008 ожидает в режиме ShareLock блокировку "транзакция 801"; заблокирован процессом 140002.
```
<img width="2560" height="1083" alt="image" src="https://github.com/user-attachments/assets/c664f25b-3010-4c8f-b63f-3e5b273b7197" />

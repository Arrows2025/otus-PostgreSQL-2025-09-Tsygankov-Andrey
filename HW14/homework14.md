...
Для выполнения домашнего задания используются три ранее установленные виртуальные машины с процессором на 1 ядро и 2 Гб оперативной памяти, на каждую виртуальную машину установлена серверная операционная система Ubuntu 24.04.3:
...
Проверка физической репликации на ВМ4: вставляю по строке в таблицу `test` на ВМ1 и в таблицу `test2` на ВМ2, новая строка по логической репликации попадает на ВМ3 и далее по физической репликации на ВМ4, при попытке вставить строку на ВМ4 выдаёт ошибку, что база данных находится в режиме только для чтения, что верно для реплики
